...
class Argument(object):
    ...
    def pruning(self, conn_node, relation, which):
        ...
        if which != 'test':
            relation.set_linker()
            prunLevelLimit = 50000

        prunLevel = 0
        while target is not None and prunLevel < prunLevelLimit:
            if target.parent_node is not None:
                siblings = target.parent_node.child_nodes
                candidate |= {(n, n.linker) for n in siblings if n != target and not is_punc(n.value)}
            # target node doesn't cover conn leaves exactly
            exact_cover = len(target.get_leaves()) == len(conn_leaves)
            if not exact_cover:
                candidate |= {(n, n.linker) for n in target.child_nodes if n != conn_node and not is_punc(n.value)}
    ...
    # TODO:: we need recover internal punctuations
    def merge(self, relation, candidates, labels):
        ...
        for label in arg_res:
            ...
            # arg_res[label] = self.recover_puncs(relation, ltmp)
            arg_res[label] = ltmp
    ...
    def print_features(self, relation, which, to_file, prev=None):
        ...
        for node, label in candidates:
            ...
            to_file_line += 'nodeCate:'+node.value+ ' '
