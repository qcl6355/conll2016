...
class ConnHeadMapper(object):
	...
	def map_raw_connective(self, raw_connective):
		...
		raw_connective_token_list = raw_connective.lower().split(' ')
                # patched by Sheng Li (case: '''typically, if''')
                tmp = []
                for conn in raw_connective_token_list:
                    tmp += conn.split(',')
                raw_connective_token_list = tmp
...
if __name__ == '__main__':
	...
	assert(indices == [1, 2 ,3])
        ...
        raw_connective = "typically, if"
        head, indices = chm.map_raw_connective(raw_connective)
        print head, indices
