...
class Relation(object):
    ...
    def init_with_annotation(self, json_str):
        ...
        self.conn_str = json_str['Connective']['RawText'].encode('utf-8')
        self.punc = json_str['Punctuation']['RawText'].encode('utf-8')
        self.punc_type = json_str['Punctuation']['PunctuationType'] if 'PunctuationType' in json_str['Punctuation'] else ""
        ...
        self.arg1_text = json_str['Arg1']['RawText'].encode('utf-8')
        self.arg2_addr = json_str['Arg2']['TokenList']
        self.arg2_text = json_str['Arg2']['RawText'].encode('utf-8')
    ...
    def get_conn_leaves(self):
        if self.rel_type == 'Explicit':
            ...
            if self.conn_str != ' '.join(n.value for n in self.conn_leaves):
                print >> logs, 'tree:%s|json:%s'% (' '.join(n.value for n in self.conn_leaves), self.conn_str)
