...
class Corpus(object):
    ...
    def collect_feature_relation_stat(frequency, parse_path, rel_path, feature_type="rule"):
        ...
        for art in Corpus.read_parses(parse_path, rel_dict):
            for rel in art.relations:
                ...
                types = {t.encode('utf-8') for t in rel.sense}
    ...
    @staticmethod
    def train_with_svm(train_file, train_vec_file, feat_map_file, model_file):
        gen_svm_train(train_file, train_vec_file, feat_map_file)
        svm_learn(train_vec_file, model_file)

    @staticmethod
    def test_with_svm(test_file, feat_map_file, test_vec_file, model_file, predict_file):
        feat_map = read_svm_map(feat_map_file)
        gen_svm_test(test_file, feat_map, test_vec_file)
        svm_classify(test_vec_file, model_file, predict_file)
...
if __name__ == '__main__':
    ...
    if feature_type == "brown":
        brown_cluster_path = FILE_PATH + '/../lib/gigaword-zh-c3200.txt'
